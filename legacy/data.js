// ============================================================================
// Bhumi Propcity CRM — seed data store (demo, local/in-memory).
...
const FIRM = (window.theme && window.theme.brand && window.theme.brand.firmName) || "Bhumi Propcity";
const SIGN = "— " + FIRM; // "— Bhumi Propcity"
...
function matchesForLead(lead) {
  ...
  const scored = properties
    ...
    .map(p => {
      ...
      return Object.assign({}, p, { _score: score, fitLine: fit.slice(0,3).join(" · ") });
    })
    .filter(p => p._score >= 3)
...
// Reverse match: which buyers/tenants want THIS property. Pass live leads.
function leadsForProperty(property, leads) {
  const scored = leads
...
function generateMessage(property, opts) {
  opts = opts || {};
  ...
  if (tone === "Short") {
    ...
    msg = [...head, "", opener, priceLine, closer, SIGN].join("\n");
  }
  return msg;
}

window.DATA = { agents, properties, leads, STAGES, matchesForLead, leadsForProperty, generateMessage, FIRM };
